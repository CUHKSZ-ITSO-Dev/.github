...
| `.github/workflows/frontend-docker-build-push.yml` | 统一前端构建后再打 Docker 镜像的流程，用于 Dockerfile 依赖预构建静态目录的项目。 | `UniAuth` |
...
业务仓库只保留触发入口、仓库路径、数据库类型、测试命令等必要参数；公共检查逻辑、工具版本和中文提示文本集中在这里维护。发布、部署、制品构建的通用机械步骤可以复用本仓库 workflow；具体部署仓库、环境路径、集群字段、对象存储端点、Secret 名称映射和内部架构说明仍由各业务仓库自行维护。
